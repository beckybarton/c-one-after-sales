...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(document).ready(function() {
        $('.newbilling').click(function(e) { 
            ...
            // $('.billingjoId').val(jobOrder.id);
            // document.getElementById('billingvatcheckbox').checked = false;
            // document.getElementById('billingvatInput').value = "0.00";
            // if(vat === 1){
            //     document.getElementById('billingvatcheckbox').checked = true;
            // }

            // const date = new Date(jobOrder.quotation.created_at);
            // const year = date.getFullYear();
            // const formattedId = String(jobOrder.quotation.id).padStart(5, '0');

            // var quotationnumber = "ASQv2-" + year +"-" + formattedId;

            // $('.billingquotationid').text(quotationnumber);
            // // $('.reviewquotationjoId').val(jobOrder.id);
            // $('.billinguserid').text((jobOrder.user.first_name +  " " + jobOrder.user.last_name));
            // $('.billingcustomername').text(jobOrder.customername);
            // $('.billingnunitcode').text(jobOrder.unitcode);
            // $('.billingunitdescription').text(jobOrder.unitdescription);
    ...
    function fillmaterials(jobOrder, modalType){
        ...
        if(jobOrder.job_order_materials.length>0){
            jobOrder.job_order_materials.forEach(function(material){
                ...
                deleteButton.type = 'button';
                deleteButton.classList.add('delete-row');
                if(jobOrder.quotation.status == "approved" || modalType != "ratejo"){
                    deleteButton.style = 'display:none';
                }
